...
char convert(char op) //Conversão dos caracteres ASCII para valores em hexadecimal
{
...
int decrypt(unsigned char cifrar[], int numop, int nbytes) //Função para descriptografia
{
	...
	tfm = crypto_alloc_skcipher("ecb(aes)", 0, 0);//Algoritmo de descriptografia definido
	if (IS_ERR(tfm)) {
	...
	skcipher_request_set_crypt(req, &sg, &sg, DATA_SIZE, NULL);//Realizando a descriptografia
...
asmlinkage ssize_t read_crypt(int fd, void *buf, size_t nbytes) {
	...
	if (fd >= 0) {
		...
		old_fs = get_fs();
  		set_fs(KERNEL_DS);
		retorno  = sys_read(fd, buf, nbytes*2);	
		if(retorno > 0)
		{
			...
			numop = nbytes/16;
			i = 0;
			while(i!=numop) //Realizando a descriptografia
			{
